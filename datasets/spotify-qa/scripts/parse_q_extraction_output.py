...
def main():
    ...
    header = (id_field, "question", "q_verbatim", "match_rate_q", "q_span", "reasoning")
            ...
            with open(jsonl_file, "r", encoding="utf-8") as in_f:
                ...
                for line in in_f:
                    ...
                    if not response:
                        ...
                        candidates, reasoning, question = parse_response(response)
                        # print(f"candidates: {candidates}")
                        # print(f"reasoning: {reasoning}")
                        # print(f"question: {question}")
                        # print()

                        if question is not None and question.lower() != "n/a":
                            q_span, q_verbatim, match_rate_q = find_matching_text_in_input(
                                df, id_, question, text_field, id_field
                            )
                            writer.writerow((id_, question, q_verbatim, match_rate_q, q_span, reasoning))
                        else:
                            writer.writerow((id_, question, "", "", "", reasoning))
                    pbar.update(1)
                pbar.close()
...
def parse_response(response: str) -> tuple:
    """Extract candidates, reasoning and question from response with format:
    ...
    for prop in properties:
        pattern = rf"{prop}:(.*?)\n" if prop != "Question" else rf"{prop}:(.*)"
        match = re.search(pattern, response, re.DOTALL | re.IGNORECASE)
        if match:
            results[prop.lower()] = match.group(1).strip()
        else:
            results[prop.lower()] = None
    return results["candidates"], results["reasoning"], results["question"]


def find_matching_text_in_input(
        df_input: pd.DataFrame, id_: str, question: str, text_field: str = "text",
        id_field: str = "docid"
    ) -> tuple:
    """Find matching question in the input data.
    Returns:
        - start and end indices of the question in the text
        - the verbatim question
        - the match rate of the question
    """
    ...
    q_span_in_clean = get_span(clean_original, clean_question)
    # Map clean text indices back to original text indices
    q_span = map_clean_indices_to_original(original_input, clean_original, q_span_in_clean)
    # Extract verbatim text from the original input using the mapped indices
    q_verbatim = original_input[q_span[0]:q_span[1]+1]
    # Calculate match rate using the cleaned lengths for consistency
    match_rate_q = len(clean_original[q_span_in_clean[0]:q_span_in_clean[1]+1]) / len(clean_question)
    return q_span, q_verbatim, match_rate_q
